...
bool GenerateDesktopFile(
	...
	const auto sourceFile = u":/misc/org.telebrain.desktop"_q;
	...
	if (!Core::UpdaterDisabled()) {
		...
		if (!Core::Launcher::Instance().customWorkingDir()) {
			QFile::remove(u"%1org.telebrain._%2.desktop"_q.arg(
				targetPath,
				md5Hash));
		...
		QFile::remove(u"%1org.telebrain.%2.desktop"_q.arg(
			targetPath,
			md5Hash));
	}
...
bool GenerateServiceFile(bool silent = false) {
	...
	if (!Core::UpdaterDisabled()
		...
		QFile::remove(u"%1org.telebrain._%2.service"_q.arg(
			targetPath,
			md5Hash));
	}
...
void start() {
	QGuiApplication::setDesktopFileName([&] {
		...
		if (!Core::UpdaterDisabled()) {
			return u"org.telebrain._%1"_q.arg(
				Core::Launcher::Instance().instanceHash().constData());
		}

		return u"org.telebrain"_q;
	}());
